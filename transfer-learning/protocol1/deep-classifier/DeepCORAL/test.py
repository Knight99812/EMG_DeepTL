...
for i in range(1, 42):
    ...
    train_loader = Data.DataLoader(dataset=train_data, batch_size=20, shuffle=True)
    ...
    for m in range(1, 3):  # validation set
        ...
        for epoch in range(1, TRAIN_EPOCHS + 1):
            model.train()
            _lambda = epoch / TRAIN_EPOCHS
            clf_loss, coral_loss = train(model, epoch, learning_rate, _lambda, train_loader, target_loader)
            coral_loss_all.append(coral_loss.data.item())
            clf_loss_all.append(clf_loss.data.item())
        endtime = time.time()
        print(endtime - starttime)

        model.eval()
        ypred, _ = model(test_xt, test_xt)
        ...
        if i == 1 & m == 1:
            ...
            test_yt_all = torch.cat((test_yt_all, test_yt), dim=0)
            pred_all = torch.cat((pred_all, pred), dim=0)
            ypred_all = torch.cat((ypred_all, ypred), dim=0)
