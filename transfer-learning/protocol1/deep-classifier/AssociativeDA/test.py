...
for i in range(1, 42):  # cross validation
    if i in nottouse:
        continue
    feature_train = []
    label_train = []
    ...
    for m in range(1, 3):  # validation set
        ...
        model = AssoDA(num_classes=10)
        visit_weight = 0.25
        walker_weight = 1
        for epoch in range(1, TRAIN_EPOCHS + 1):
            model.train()
            fit(model, epoch, learning_rate, visit_weight, walker_weight, train_loader, target_loader)

        model.eval()
        _, ypred = model(test_xt)
