function [acc,pred,time_pass] = CORAL_SVM(Xs,Ys,Xt,Yt)
...
%% Please download libsvm and add it to Matlab path before using SVM.
    ...
    [pred,acc] = SVM_Accuracy(double(Xs), A_coral, double(Yt), Sim_coral, double(Ys));
    time_end = clock();
    time_pass = etime(time_end,time_start);
end

function [pred,acc] = SVM_Accuracy (trainset, M,testlabelsref,Sim,trainlabels)
    % Using Libsvm
    Sim_Trn = trainset * M *  trainset';
    index = [1:1:size(Sim,1)]';
    Sim = [[1:1:size(Sim,2)]' Sim'];
    Sim_Trn = [index Sim_Trn ];
    model = svmtrain(trainlabels, Sim_Trn,'-s 0 -t2 -c 0.5 -g 0.1');
    [pred, acc, ~] = svmpredict(testlabelsref, Sim,model);
    acc =acc(1)
