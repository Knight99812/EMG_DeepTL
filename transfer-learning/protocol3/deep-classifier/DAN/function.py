...
def train(model, loss_func, epoch, learning_rate, source_loader, target_loader):
    ...
    for i in range(1, num_iter):
        ...
        a1, a2, a3, out1, b1, b2, b3, out2 = model(source_data, target_data)
        clf_loss = clf_criterion(out1, source_label)
        loss1 = loss_func(a1, b1)
        loss2 = loss_func(a2, b2)
        loss3 = loss_func(a3, b3)
        loss4 = loss_func(out1, out2)
        sum_loss = clf_loss + loss1 + loss2 + loss3 + loss4
        ...
        if i % log_interval == 0:
            print('Train Epoch {}: [{}/{} ({:.0f}%)]\tLoss: {:.6f}\tclf_Loss: {:.6f}\tfc1_Loss: {:.6f}\tfc2_Loss: {:.6f}\tfc3_Loss: {:.6f}\tout_Loss: {:.6f}'.format(
                epoch, i * len(source_data), len(source_loader) * BATCH_SIZE,
                       100. * i / len(source_loader), sum_loss.data.item(), clf_loss.data.item(),
                loss1.data.item(), loss2.data.item(), loss3.data.item(), loss4.data.item()))
